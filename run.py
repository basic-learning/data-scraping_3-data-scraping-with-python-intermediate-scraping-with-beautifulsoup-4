...
def login():
    print('login....')
    datas = {'username':'user', 'password':'user12345'}
    res = session.post('http://167.172.70.207:9999/login',data=datas)
    f = open('./res.html','w+')
    f.write(res.text)
    f.close()
...
def get_urls(page):
    ...
    params = {'page': page}
    res = session.get('http://167.172.70.207:9999',params=params)
...
def get_detail(url):
    ...
    res = session.get('http://167.172.70.207:9999'+url)
